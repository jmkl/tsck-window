...
use crate::{
    col,
    statusbar_manager::{
        SlotDiv, SlotText, StatusBar, StatusBarFont, Visibility, WM_UPDATE_STATUSBAR,
        get_statusbar_height,
    },
};
...
pub struct WidgetSlots {
    ...
    pub divs: BTreeMap<String, Vec<SlotDiv>>,
...
impl WidgetSlots {
    ...
    pub fn set_multiline(&mut self, key: &str, slot: Vec<SlotDiv>) {
        self.divs.insert(key.into(), slot);
        self.refresh_statusbar();
    }
    ...
    pub fn refresh_statusbar(&mut self) {
        ...
        let multiline = self.divs.values().flatten().cloned().collect();
